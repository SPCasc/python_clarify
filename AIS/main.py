...
def index():
    return render_template_string('''
        ...
        <form action='/upload' method='POST' enctype='multipart/form-data'>
            ...
            <label for='campo_selic'> Arquivo da Taxa da Selic (CSV) </label>
        ...
        <a href='/editor_inadimplencia'> Editar dados de Inadimplencia </a></br>
...
def consultar():
    ...
    # cadeira no meio da vaga
    if request.method == 'POST':
        tabela = request.form.get('campo_tabela')
        if tabela not in ['inadimplencia', 'selic']:
            return jsonify({"Erro":"Você está tentnado me invadir! Aqui não!!!"}),
        400
        with sqlite3.connect(DB_PATH) as conn:
